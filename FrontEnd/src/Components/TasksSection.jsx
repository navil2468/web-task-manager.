...
function TasksSection() {
  ...
  const addTask = async () => {
    if (input.trim()) {
      try {
        const res = await axios.post('http://localhost:5001/tasks', { title: input });
  ...
  const toggleTask = async (id, completed) => {
    try {
      const res = await axios.put(`http://localhost:5001/tasks/${id}`, { completed: !completed });
    ...
    <div className='bg-neutral-700 m-10 p-5 rounded-xl'>
      <div className='lg:flex justify-between'>
        <div className='pl-3'>
          ...
          <h1 className='text-xl pl-5 pt-1 text-neutral-400'>
        ...
        <div className='lg:text-5xl text-3xl lg:pt-0 pt-5 flex pr-5'>
          <input
            ...
            className='border-amber-500 rounded-l-xl border py-2 focus:outline-none focus:outline-neutral-100 focus:ring-1 pl-5'
          />
          <button
            onClick={addTask}
            className='px-5 border rounded-r-lg border-amber-500 bg-amber-500 hover:bg-orange-500 hover:border-orange-500 cursor-pointer'>
            ➕
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <ul className='mt-10 space-y-5 mx-10'>
          {tasks.map((task) => (
            <li key={task._id} className='flex items-center justify-between text-5xl text-neutral-800 bg-neutral-400 rounded-xl pl-5 py-5'>
              ...
              <button className='pr-5' onClick={() => deleteTask(task._id)}>
